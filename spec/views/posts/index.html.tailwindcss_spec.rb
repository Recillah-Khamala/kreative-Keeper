...
RSpec.describe "User posts view", type: :feature do
    before(:example) do
      @user = User.create(name: 'User1', photo: 'https://source.unsplash.com/user/c_v_r/100x100', bio: 'Ruby on Rails enthusiast', postscounter: 2)
      @posts = 5.times.collect do |i|
        Post.create(title: "sample post #{i + 1}", text: 'Rails all the way', commentscounter: 0, likescounter: 0, author_id: @user.id)
      end
      @post = @posts.first
